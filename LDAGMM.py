        ...
        if line[0].find('d') >-1:
            ...
            for word in words:
                ...
                if word == ' ':
                    ...
                    wordcount[word] = 1
    fileopen.close
    file_w = open('init/' + name + '/wordcount', 'wb')
...
def init_with_stop_and_frequence():
    countword()
    #读取单词词频文件
    file_r = open('init/' + name + '/wordcount', 'rb')
    ...
    for line in file_r:
        line = line.strip()
        line = line.split('::')
        if line[0].find('d') >-1 or line[0].find('q')> -1 or line[0].find('s')> -1:
            print line[0]
            updatedoc.append(line[0])
            if len(doc) >  0:
                #docs.append(doc)
    ...
    #cPickle.dump(worddic, file_w)
    #file_w.close()
    file_w = open('init/' + name + '/dswvector', 'wb')
    cPickle.dump(dswvector, file_w)
    file_w.close()
    fileopen.close()
...
#进行GMM算法
def ldagmm():
    file_r = open('init/' + name + '/dswvector', 'rb')
...
def iter():   #进行迭代
    ...
    for t in range(Iteration):
        ...
        for nameofupdatedoc in updatedoc:
            ...
            file_r = open('init/' + name + '/' + nameofupdatedoc, 'rb')
            ...
            for s in range(len(doc[2])):
                ...
                for w in range(len(sen[2])):
                    ...
                    vious = diffgmm(score,doc[1],g.d_topic,g.means_,g.covars_) + diffsigmoid(doc,s,w,alldata,item[1])
                    ...
                    vious = diffgmm(score,sen[1],s_topic,g.means_,g.covars_) + diffsigmoid(doc,s,w,alldata,item[2])
            ...
            #更新完毕，对结果进行保存
            file_w = open('init/' + name +'/' + nameofupdatedoc, 'wb')
            cPickle.dump(doc, file_w)
            file_w.close()
            #file_w = open('LDAGMM/worddic', 'wb')
...
def iter_with_LDA():  # 进行迭代
    ...
    for t in range(Iteration):
        ...
        covars = g.covars_
        predict = g.predict(alldataarray)
        file_w = open('init/' + name + '/weight', 'wb')
        cPickle.dump(g.weights_, file_w)
        file_w.close()
        file_w = open('init/' + name + '/predict', 'wb')
        cPickle.dump(predict, file_w)
        file_w.close()
        file_w = open('init/' + name + '/mean', 'wb')
        cPickle.dump(g.means_, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name + '/weight' + str(t) + '.txt', 'w')
        file_w.write(str(g.weights_).replace('\n', '') + '\n')
        file_w.close()
        file_w = open('updateLDA/' + name + '/predict' + str(t) + '.txt', 'w')
        file_w.write(str(predict).replace('\n', '') + '\n')
        file_w.close()
        file_w = open('updateLDA/' + name + '/mean_' + str(t) + '.txt', 'w')
        for i in range(K):
            file_w.write(str(g.means_[i]).replace('\n', '') + '\n')
        file_w.close()
        ...
        svector = {}  #存放句子的字典
        sdistribution = {}  # 存放句子在每一个高斯成分上的分布
        ddistribution = {}  # 存放句子在每一个高斯成分上的分布
        wvector = {}  # 存放单词的字典
        wdistribution = {}  # 存放单词在每一个高斯成分上的分布
        ...
        for topicindex, nameofupdatedoc in enumerate(updatedoc):
            ...
            doc = cPickle.load(file_r)
            file_r.close
            ddistribution[doc[0]] = g.predict_proba([doc[1]])
            for s in range(len(doc[2])):
                sen = doc[2][s]
                sdistribution[sen[0]] = g.predict_proba([sen[1]])
                for w in range(len(sen[2])):
                    ...
                    vious = diffgmm(score, sen[1], s_topic, g.means_, g.covars_) + diffsigmoid_with_LDA(doc, s, w,alldata,d_topic, item[2])
                    sen[1] = sen[1] + currenta * vious
                    doc[2][s] = sen
                    alldata[sen[0]] = sen[1]
                    svector[sen[0]] = sen[1]
        ...
        cPickle.dump(alldata, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name +'/svector', 'wb')
        cPickle.dump(svector, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name + '/dvector', 'wb')
        cPickle.dump(dvector, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name + '/wvector', 'wb')
        cPickle.dump(wvector, file_w)
        file_w.close()
        # 写入更新的文档，句子，单词的更新向量
        file_w = open('updateLDA/' + name + '/dvector' + str(t) + '.txt', 'w')
        dkeys = dvector.keys()
        for key in dkeys:
            file_w.write(key + '/t' + str(dvector[key]).replace('\n', '') + '\n')
        file_w.close()

        file_w = open('updateLDA/' + name + '/svector' + str(t) + '.txt', 'w')
        skeys = svector.keys()
        for key in skeys:
            file_w.write(key + '/t' + str(svector[key]).replace('\n', '') + '\n')
        file_w.close()

        file_w = open('updateLDA/' + name + '/wvector' + str(t) + '.txt', 'w')
        wkeys = wvector.keys()
        for key in wkeys:
            file_w.write(key + '/t' + str(wvector[key]).replace('\n', '') + '\n')
        file_w.close()
        # 对句子，单词在每个高斯成分上的分布进行更新
        file_w = open('updateLDA/' + name + '/ddistribution', 'wb')
        cPickle.dump(ddistribution, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name + '/sdistribution', 'wb')
        cPickle.dump(sdistribution, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name + '/wdistribution', 'wb')
        cPickle.dump(wdistribution, file_w)
        file_w.close()
        file_w = open('updateLDA/' + name + '/ddistribution' + str(t) + '.txt', 'w')
        dkeys = ddistribution.keys()
        for key in dkeys:
            file_w.write(key + '/t' + str(ddistribution[key]).replace('\n', '') + '\n')
        file_w.close()

        file_w = open('updateLDA/' + name + '/sdistribution' + str(t) + '.txt', 'w')
        skeys = sdistribution.keys()
        for key in skeys:
            file_w.write(key + '/t' + str(sdistribution[key]).replace('\n', '') + '\n')
        file_w.close()

        file_w = open('updateLDA/' + name + '/wdistribution' + str(t) + '.txt', 'w')
        wkeys = wdistribution.keys()
        for key in wkeys:
            file_w.write(key + '/t' + str(wdistribution[key]).replace('\n', '') + '\n')
        file_w.close()
